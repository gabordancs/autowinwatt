"""Open and cancel the XML Import file picker without selecting a file."""
...
def _open_xml_import(main_window: Any) -> None:
    ...
    file_menu.click()
    time.sleep(0.1)
    next(item for item in file_menu.sub_menu().items() if int(item.item_id()) == 10).click()


def _find_open_dialog(process_id: int, *, timeout: float) -> Any | None:
    from pywinauto.application import Application
...
def run_safe_xml_import_probe(*, dialog_timeout: float = 3.0, close_timeout: float = 2.0) -> dict[str, Any]:
    """Verify the picker while forbidding path selection and import confirmation."""
    prepare_main_window_for_menu_interaction()
    main_window = ensure_main_window_foreground_before_click(action_label="safe_xml_import_probe", allow_dialog=True)
    _open_xml_import(main_window)
    ...
    return {
        ...
        "command": "MainForm.XMLImportAction",
        "native_menu_path": [{"menu_command_id": 1}, {"command_id": 10}],
        "dialog_found": found,
        ...
        "forbidden_effect": "select_file_or_confirm_xml_import",
