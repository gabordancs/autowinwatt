...
class CertificateProjectBuilder:
    ...
    def build(self,request: CertificateBuildInput) -> CertificateBuildResult:
        ...
        manifest={"source":{"pdf":str(pdf),"sha256":source_digest(pdf),"pages":len(pages)},"values":[item.model_dump(mode="json") for item in values],"material_decisions":[item.model_dump(mode="json") for item in decisions],"unresolved":unresolved,"llm_used":llm_used,"llm_result":llm_result,"geometry_contract":{"vertical_wall":{"x_m":"plan-measured wall length", "y_m":"room wall height", "area_m2":"x_m*y_m", "evidence_required":"room contour + exterior contour + dimension chain"}},"next_step":"Provide an approved structured geometry mapping before native XML generation. Every vertical wall must supply X length, Y height and A=X*Y; raw PDF prose is insufficient evidence for room-boundary allocation."}
