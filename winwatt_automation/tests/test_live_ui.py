...
def test_focus_guard_allows_safe_project_open_probe_when_exists_false_but_identity_strong(monkeypatch):
    main_window = FakeWindow(
        title="WinWatt gĂłlya",
        class_name="TMainForm",
        process_id=1048,
        handle=4328702,
        exists=False,
    ...
    monkeypatch.setattr(
        app_connector,
        "describe_foreground_window",
        lambda: {"handle": 4328702, "title": "WinWatt gĂłlya", "class_name": "TMainForm", "process_id": 1048},
    )

    resolved = app_connector.ensure_main_window_foreground_before_click(action_label="safe_project_open_probe", timeout=0.01)
