...
def test_get_cached_main_window_materializes_window_spec_wrapper(monkeypatch):
    class MaterializedWindow(FakeWindow):
        pass

    resolved = MaterializedWindow(title="WinWatt - Project", class_name="TMainForm", process_id=999, handle=444)

    class FakeWindowSpecification:
        def wrapper_object(self):
            return resolved

    app_connector.WinWattSession.main_window = FakeWindowSpecification()
    app_connector.WinWattSession.process_id = 999
    app_connector.WinWattSession.handle = 444
    app_connector.MainWindowSession.last_validation_monotonic = 0.0
    app_connector.MainWindowSession.validation_interval_s = 100.0

    monkeypatch.setattr(app_connector.time, "monotonic", lambda: 1.0)

    result = app_connector.get_cached_main_window()

    assert result is resolved
    assert app_connector.WinWattSession.main_window is resolved


def test_get_cached_main_window_refocuses_before_resolve(monkeypatch):
    cached_window = FakeWindow(title="WinWatt - Project", class_name="TMainForm", process_id=999, handle=444)
    app_connector.WinWattSession.main_window = cached_window
    app_connector.WinWattSession.process_id = 999
    app_connector.WinWattSession.handle = 444
    ...
    calls = {"health": 0, "focus": 0}

    def fake_health(window):
        calls["health"] += 1
        if calls["health"] == 1:
            return False, "foreground_context_failed"
        return True, "healthy"

    def fake_focus(window, *, maximize=False):
        calls["focus"] += 1

    monkeypatch.setattr(app_connector.time, "monotonic", lambda: 100.0)
    monkeypatch.setattr(app_connector, "_cached_window_health_status", fake_health)
    monkeypatch.setattr(app_connector, "_best_effort_focus_window", fake_focus)
    monkeypatch.setattr(app_connector, "_resolve_uia_main_window", lambda: (_ for _ in ()).throw(AssertionError("resolve should not run after successful refocus")))

    result = app_connector.get_cached_main_window()

    assert result is cached_window
    assert calls == {"health": 2, "focus": 1}
    assert app_connector.MainWindowSession.foreground_failure_count == 0
